...
func TestProxyAddAndEdgePairs(t *testing.T) {
    ...
    person := m.Get("Person")
    if person == nil { t.Fatal("Person schema missing") }
    p := NewEntityProxy(person, "p1")
    if err := p.Add("name", []string{" John  Smith "}, false, ""); err != nil {
        t.Fatalf("add name: %v", err)
    }
    vals := p.Get("name", false)
    if len(vals) != 1 || vals[0] != "John Smith" { t.Fatalf("unexpected name values: %v", vals) }

    // Relationship entity: Ownership (edge)
    own := m.Get("Ownership")
    if own == nil { t.Skip("Ownership schema missing") }
