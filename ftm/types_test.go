...
func TestCountryAndLanguageClean(t *testing.T) {
    ...
    if !ok || out != "de" { t.Fatalf("country clean failed: %v %v", ok, out) }

    l := NewLanguageType()
    out, ok = l.Clean("DEU", false, "", nil)
    if !ok || out != "deu" { t.Fatalf("language clean failed: %v %v", ok, out) }
}
...
func TestIdentifierIBANAndBIC(t *testing.T) {
    ...
    if !ok || bic != "DEUTDEFF" { t.Fatalf("bic clean failed: %v %v", ok, bic) }
}

func TestURLNormalizationAndCompare(t *testing.T) {
    u := NewURLType()
    a, ok := u.Clean("Example.com/Path?b=2&a=1#frag", false, "", nil)
    if !ok { t.Fatalf("url clean failed") }
    b, ok := u.Clean("http://example.com/Path?a=1&b=2", false, "", nil)
    if !ok { t.Fatalf("url clean failed (b)") }
    if u.Compare(a, b) < 1.0 { t.Fatalf("url compare expected equal: %s vs %s", a, b) }
}

func TestNameAndAddressCompare(t *testing.T) {
    n := NewNameType()
    if n.Compare("John Smith", "J. Smith") <= 0.0 { t.Fatalf("name compare too low") }
...
func TestPhoneCleanWithCountryHint(t *testing.T) {
    ...
    ps := m.Get("Person")
    if ps == nil { t.Skip("Person schema missing") }
    p := NewEntityProxy(ps, "p1")
    // Add nationality first, so phone has country hint
    if err := p.Add("nationality", []string{"de"}, false, ""); err != nil { t.Fatalf("add nationality: %v", err) }
    // Now add phone
    ph := NewPhoneType()
    out, ok := ph.Clean("030 1234567", false, "", p)
    if !ok || out == "" { t.Fatalf("phone clean failed: %v %v", ok, out) }
    if out[0] != '+' { t.Fatalf("expected E.164, got: %s", out) }
}

func TestChecksumValidation(t *testing.T) {
    cs := NewChecksumType()
    if cs.Validate("DEADbeef") { t.Fatalf("short checksum should be invalid") }
    if !cs.Validate("0123456789abcdef0123456789abcdef01234567") { t.Fatalf("valid checksum failed") }
}

func TestJsonType(t *testing.T) {
    jt := NewJsonType()
    if !jt.Validate(`{"a":1}`) { t.Fatalf("json validate failed") }
    // raw string input should be JSON-string-encoded on clean
    out, ok := jt.Clean("hello", false, "", nil)
    if !ok || out != "\"hello\"" { t.Fatalf("json clean string failed: %v %v", ok, out) }
}
